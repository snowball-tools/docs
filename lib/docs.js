...
import { topicById, headerById } from "./docs-chapters.mjs";
...
export function renderChapterContent(source) {
  return marked
    ...
    .replace(/<chapter>(.*?)<\/chapter>/g, (_, body) =>
      renderChapterLink(...body.trim().split("#"))
    )
    .replace(/<chapter href="(.+?)">(.*?)<\/chapter>/g, (_, href, body) =>
      renderCustomChapterLink(href, body.trim())
    )
...
function wrapAside(body) {
  ...
  <div class="AsideContent">
    ...
    <div>Wrapping Aside with body</div>
  </div>
</aside>
`;
}
...
function renderChapterLink(id, hash) {
  ...
  if (!header.href) {
    return `<span class="ChapterLink -coming-soon"><span class="emoji">${topic.emoji}</span>&nbsp;<span class="title">${header.title}</span>
    <span class="coming-soon">(coming soon logic)</span></span>`;
  } else {
    return `<span class="ChapterLink" href="${
      header.href
    }"><span class="emoji">${topic.emoji}</span>&nbsp;<a href="${header.href}${
      hash ? "#" + hash : ""
    }" class="title">${header.title}</a></span>`;
  }
}
