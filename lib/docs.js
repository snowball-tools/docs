  ...
  return `
    <div class="flex items-start">
      ...
      <div class="hidden lg:block sticky top-0">
        <div class="prose !p-0 !pt-6">
          <h1 class="!text-sm !font-bold">Table of Contents</h1>
          ${toc}
        </div>
      </div>
    </div>
...
renderer.renderer.rules.fence = function(tokens, idx, options, env, self) {
  ...
  let content
  if (langShort && hljs.getLanguage(langShort)) {
    try {
      content = hljs.highlight(token.content, { language: langShort }).value
    } catch (__) {
      content = renderer.utils.escapeHtml(token.content)
    }
  }
  else {
    content = renderer.utils.escapeHtml(token.content)
  ...
  return `
    <div class="FancyCodeBlock px-1 pb-1 bg-slate-100 rounded-xl flex-col justify-start items-center flex">
      ...
      <div class="self-stretch p-4 bg-white rounded-lg shadow-m justify-start items-center gap-1.5 inline-flex overflow-x-auto">
        <pre class="not-prose text-sm"><code data-copy-id="${copyId}">${content}</code></pre>
      </div>
    </div>
  `
}
