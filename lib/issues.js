...
const BASE_REQUIRED_FIELDS = ['content', 'topic']
...
const IMPACT_SEVERITIES = ['critical', 'spec', 'high', 'medium', 'low']

const CHANCE_SEVERITIES = ['high', 'medium', 'low', 'na']
...
export function validate(issue) {
  ...
  if (!(valid = Object.keys(STATUSES)).includes(issue.status) && issue.severity !== 'I') {
    throw new Error(
      `Invalid issue status: '${issue.status}'. Valid options: ${valid.join(', ')} (for issue: ${estimateIssuePreview(issue)})`,
    )
  }
  if (!(valid = IMPACT_SEVERITIES).includes(issue.impact) && issue.severity !== 'I') {
    throw new Error(
      `Invalid issue impact: '${issue.impact}'. Valid options: ${valid.join(', ')} (for issue: ${estimateIssuePreview(issue)})`,
    )
  }
  if (issue.status === 'fixed' && !issue.commit && !issue.commitTree && !issue.customCommits) {
    throw new Error(
      `Fixed issue missing 'commit', 'commitTree', or 'customCommits' field (for issue: ${estimateIssuePreview(issue)})`,
    )
  } else if (issue.status === 'fixedprivate' && (issue.commit || issue.commitTree)) {
    throw new Error(
      `FixedPrivate issue cannot have 'commit' or 'commitTree' field (for issue: ${estimateIssuePreview(issue)})`,
    )
  } else if (
    (issue.commit || issue.commitTree || issue.repoPath) &&
    issue.status !== 'fixed' &&
    issue.status !== 'addressed'
  ) {
    throw new Error(
      `Do not provide a 'commit' or 'commitTree' field when issue is not 'fixed' or 'addressed' (for issue: ${estimateIssuePreview(issue)})`,
    )
  }

  if (issue.isVuln && !issue.chance) {
    throw new Error(
      `'chance' field required for High, Medium, and Low severity issues (for issue: ${estimateIssuePreview(issue)})`,
    )
  } else if (!issue.isVuln && issue.chance) {
    throw new Error(
      `Do not use the 'chance' field for Code Quality or Gas Opt issues (for issue: ${estimateIssuePreview(issue)})`,
    )
  }
  ...
  return markdown
    ...
    .map((issueContent) => {
      ...
      if (chanceMatch) {
        ...
        chance = chanceMatch[1].trim().match(/^n\s*\/\s*a$/i)
          ? 'na'
          : chanceMatch[1].toLowerCase().trim()
...
export function notionIssueToLancerCollectionItem(issue) {
  ...
  `)
    .replace('__TOPIC__', `<field name="topic">${issue.topic || 'TODO'}</field>`)
    .replace(
      '__IMPACT__',
...
function validateBaseRequiredFields(issue, fields) {
  for (let field of fields) {
    if (!issue[field]) {
      throw new Error(
        `Issue missing required field: '${field}' for issue: ${estimateIssuePreview(issue)}`,
      )
    }
  }
}
